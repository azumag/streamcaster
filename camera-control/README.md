...
- **プリセット**：会場IDごとに8枠、最大16会場。保存・呼出に必要な条件（位置未受信・Zoom未受信・接触喪失・未ARM）は、ボタンを押す前にプリセット欄の上へ常時表示します。OSCで受信した位置とZoomをサーバーの`.data/presets.json`へ保存。上書きは5秒以内に再度保存を押します（拒否された場合、この確認は消費されません）。保存は最後に受信した位置とZoomを記録するため、カメラを構えたまま止めていても保存できます。保存が成立すると緑のトーストが出て、プリセットカードのZoom・座標表示が更新されます（同じ名前で上書きすると画面上の変化が無く、拒否と見分けられなかったため）。呼出はCUTまたは指定秒数の補間です。公式Camera Dollyのファイル再生ではありません。保存には受信Zoomが必要です。呼出側では、CUTは保存済みZoomへそのまま切り替わるため受信Zoomを必要としませんが、秒数指定の補間は開始Zoomが要るためVRChat側でZoomを一度動かしてください。

- **撮影プレビュー**：「撮影」で`/usercamera/Capture`を送り、VRChatに写真を保存させます。**移動が終わってカメラが静止すると自動で1枚撮ります**（既定でON、チェックで解除）。操作者はカメラの映像を見られないため、「本当にそこへ行けたのか」を答える手段が他にありません。静止から0.8秒待ってから撮り、その前に再び動き出した場合は撮りません（途中経過の写真は何も確認できないため）。STOPや接触喪失、ARM解除では撮りません。1秒に1枚までです。UIは`--photo-dir`の**最新の1枚だけ**を表示します。ブラウザからファイル名・パスは指定できません。URL末尾の数字は更新用のキャッシュキーで、サーバーは常に自分が見つけた最新ファイルを返します。連写でディスクを埋めないよう1秒に1枚までです。書き込み途中のファイルを割れた画像として出さないため、保存直後の1秒弱は表示が遅れます。シンボリックリンクは読みません。**映像モニターの代わりではなく、構図確認用の静止画です。**
...
- [ ] 移動後の自動撮影が、プリセット呼出・手動PTZの両方で1枚だけ撮り、着地後の構図が写っていることを確認する。
