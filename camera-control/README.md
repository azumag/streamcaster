...
**誰が操作してよいかはTailscale側のgrants/ACLで決めます。** 本アプリはユーザー単位の許可リストを持たず、tailnetから到達できる利用者を操作可能として扱います。役職別の権限管理は未実装です。操作権を取れるのは同時に一人だけですが、**他の接続者が明示操作で奪うことはできます**。役職別に奪える人を制限する仕組みはありません。**STOPだけはVRChat PC（手元の8765）からのみ使えます。** 遠隔の担当者にSTOPはありません。カメラを触らせたくない相手には、このPCのTCP 8443へ到達できないようACLを設定してください。
...
手動入力が約0.4秒途絶えると送信を止めます。WebSocket切断・操作権返却・操作権の移動では**移動を停止しますが、ARMは維持します**。STOP操作、受信したMode変更、Lock/LookAtMe有効化ではARM解除します。制御ループが0.2秒を超えて止まった場合は、**移動中だったときだけ**停止・ARM解除します。静止していたカメラの位置は遅延で変わらないため、ARMを維持します。
...
**STOPはVRChat PCからのみです。** 非常停止は、画面と現場を実際に見ている人のものであって、写真越しに操作している遠隔の担当者のものではありません。判定は接続が届いたリスニングポートで行うため（身元ヘッダやOriginと違い、クライアントが詐称できません）、遠隔ポート経由の接続ではSTOPボタンが無効になり、サーバーも拒否します。遠隔の担当者が自分の起こした移動を止めたい場合は「操作権を返す」で移動が停止します（ARMは維持）。手動入力は0.4秒で止まるため、止まらないのはプリセット移動だけです。

**他の担当者が操作中でも、操作権を奪えます。** 「操作権を取得」は相手が操作中なら失敗します（ページは開いた時点で自動取得するため、タブを開いただけで運転中のカメラを奪ってはいけません）。奪うのは「操作権を奪う」という明示の操作で、**5秒以内に2回押す**必要があります。奪われた側にはトーストで誰に奪われたかが出ます。サーバーのログにも `takeover: accepted` として残ります。奪っても移動は引き継がれず、ARMは維持されます。

**操作権が移ってもARMは引き継がれます。** ARMは「カメラの位置が分かっている」という状態であって、誰が操作しているかとは無関係です。担当者が交代するたびに取り直させる理由がありません。**移動は引き継ぎません**：軸入力・速度・補間はすべて停止し、次の操作者が動かし始める時点で最後の受信位置へ再同期します（古い目標を引き継いだ移動は起こりません）。操作権を持たない状態では誰も動かせないので、ARMが維持されていても無主のカメラが動くことはありません。
...
これらはアプリのタイマー設定であり、実ネットワークやOS停止時の遅延保証ではありません。**STOPは新しい絶対Poseの送信を止めるもので、VRChat内蔵の平滑化の残り、他の操作ツール、OBS配信を強制停止しません。** またSTOPはVRChat PCからのみ使えます。 UDPに受領保証はありません。VRChat側で手動操作できる人と確認映像を残してください。
