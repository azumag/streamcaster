...
2026-09-15に再確認したVRChat公式2025.3.3リリースノートは `/usercamera/Pose` を **Get/Set** と説明していますが、公式Wikiは同じアドレスを **read-only** としています。記載が食い違うため、位置・回転の送信は `--enable-pose-write` を指定した場合だけ有効になる**試験機能**です。
...
- **このフラグはVRChat側の制限を解除しません。** 実クライアントがPoseを読み取り専用として扱う場合、位置移動はできません。反映しなければSTOPし、この経路は非対応として扱ってください。
...
`http://127.0.0.1:8765` を開き、起動ターミナルに表示された `Camera access token` を入力します。トークン未設定なら起動ごとにランダム生成します。URLには含めず、ブラウザストレージにも保存しません。ターミナルを配信に映さないでください。

固定トークンが必要な場合は十分ランダムな32〜256バイトの値を環境変数 `CAMERA_CONTROL_TOKEN` に設定します。既存の `BLMF_OPERATOR_TOKEN` や配信キーは流用しないでください。再起動時に操作権や移動を自動復元しません。
...
VRChatのAction Menuで **OSC → Enabled** を有効化し、カメラを開いて少し動かし、Zoomも操作します。Web側で受信値を確認して「操作権を取得」→「受信した位置でARM」の順に押してください。Lock / Look At MeはVRChat側でもOFFにします。ARM時は5秒以内に受信したPoseが必要です。OSCが変更時だけ送信される場合もあるため、無受信だけで接続断とは断定しません。
...
担当者は `https://vrchat-pc.example-tailnet.ts.net:8443` を開き、個別に渡したカメラ専用トークンで接続します。`--public-origin` は末尾スラッシュなしの正確なOriginです。環境変数 `CAMERA_PUBLIC_ORIGIN` でも指定できます。未指定のホスト名・Origin・クエリ文字列は拒否します。
...
本アプリは共有トークン認証です。TailscaleユーザーIDとの自動連携、役職別権限管理は未実装です。認証済み接続のうち操作権を取れるのは同時に一人だけで、他の認証済み利用者もSTOPできます。信頼した担当者だけにトークンを渡してください。
...
- **プリセット**：会場IDごとに8枠、最大16会場。OSCで受信した位置とZoomをサーバーの`.data/presets.json`へ保存。上書きは5秒以内に再度保存を押します。呼出はCUTまたは指定秒数の補間です。公式Camera Dollyのファイル再生ではありません。
...
手動入力が約0.4秒途絶えると送信を止めます。操作権のheartbeatが約1.5秒途絶える、WebSocket切断、操作権返却、フォーカス喪失・タブ非表示、制御ループの0.2秒超の停止を検知すると移動停止／ARM解除します。Pose受信が5秒以上古い場合、受信したMode変更、Lock/LookAtMe有効化でも解除します。
...
WebSocketはOrigin/Host制限、最初のトークン認証、最大4KBメッセージ、接続数16、毎秒80メッセージ／接続の上限を設けています。遅い閲覧者への状態送信はモーションループから分離しています。ブラウザが任意のOSCアドレス・送信ホスト・ファイルパス・シェルコマンドを指定するAPIはありません。
...
- [ ] 別の担当者からTailscale HTTPS/WSSで操作し、未許可デバイスの接続拒否を確認する。
