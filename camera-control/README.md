...
手動入力が約0.4秒途絶えると送信を止めます。WebSocket切断、操作権返却、フォーカス喪失・タブ非表示を検知すると移動停止／ARM解除します。受信したMode変更、Lock/LookAtMe有効化でも解除します。制御ループが0.2秒を超えて止まった場合は、**移動中だったときだけ**停止・ARM解除します。静止していたカメラの位置は遅延で変わらないため、ARMを維持します。
