...
- **撮影プレビュー**：「撮影」で`/usercamera/Capture`を送り、VRChatに写真を保存させます。**移動が終わってカメラが静止すると自動で1枚撮ります**（既定でON、チェックで解除）。操作者はカメラの映像を見られないため、「本当にそこへ行けたのか」を答える手段が他にありません。静止から0.8秒待ってから撮り、その前に再び動き出した場合は撮りません（途中経過の写真は何も確認できないため）。STOPや接触喪失、ARM解除では撮りません。1秒に1枚までです。撮影を指示してからファイルが現れるまではプレビュー上に「撮影中…」を出します（前の写真が新しい写真に見えるのを防ぐため）。10秒経っても保存されなければ待つのをやめます。
